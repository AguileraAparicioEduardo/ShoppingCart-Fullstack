  ...
  return (
    <div className='relative'>
      <picture>
        ...
        <img
          className={`rounded-lg mb-[38px] w-full h-[200px] object-cover ${quantity > 0 ? 'border-2 border-Red' : ''}`}
          src={image.mobile}
          alt={name}
        />
      </picture>
      ...
      {quantity === 0 ? (
        <button
          ...
          className='bg-Rose-50 border-2 border-Rose-300 w-40 rounded-full flex justify-center gap-2 p-3 absolute inset-x-0 mx-auto top-[170px] cursor-pointer hover:border-Red transition-colors'
        >
          <img src='/assets/images/icon-add-to-cart.svg' alt='' />
          <span>Add to Cart</span>
        </button>
      ) : (
        <div className='bg-Red w-40 rounded-full flex justify-between items-center p-3 absolute inset-x-0 mx-auto top-[170px]'>
